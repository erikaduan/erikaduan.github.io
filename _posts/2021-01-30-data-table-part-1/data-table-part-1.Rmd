...
In R, datasets exist as `data.frame` type objects. To apply `data.table` functions on a dataset, we need to convert a `data.frame` into a `data.table` object using the `setDT` function.   
...
You can use the R package `microbenchmark` and Python package `timeit` to measure code performance. ^[To print more accurate benchmarks for this blog, I had to save the values produced by `microbenchmark()` and explicitly print them out, rather than rely on outputs produced during document knitting.]       
...
In `data.table`, data sorting is also performed inside `i` of `DT[i, j, by]`.   
Placing the operator `-` in front of a variable enables sorting by descending order.     
...
Interestingly, the benchmarking results show that `dplyr` actually performs slightly faster than `data.table` for column selections.        
...
In `dplyr` version >= 1.0.0, we can use `mutate()` in combination with `across()` to create new columns or apply transformations across one or multiple columns. Copy-on-modify behaviour occurs whenever a column is modified and we must reassign the data frame a name to retain any changes.    

```{r}
#-----create column(s) using dplyr-----  
# create new columns 
student_courses <- student_courses %>%
  mutate(platform_dwell_length = difftime(platform_end_date, platform_start_date, units = "days"),
         platform_start_year = as.numeric(str_extract(platform_start_date, "^.{4}(?!//-)")))

# create a new column by evaluating multiple conditions  
...
student_courses[,
                c("platform_dwell_length",
                  "platform_start_year") := .(difftime(platform_end_date, platform_start_date, units = "days"),
...
# use subassignment if you want to subset just the transformed columns   
...
When working with Pandas DataFrames, column-based data transformations can be applied using:  

+ By directly transforming column(s).     
+ Using `apply()` with anonymous lambda functions i.e. `df['col1'].apply(lambda x: x + 1, axis = 0)`.    
+ Using the optimised `applymap()` with anonymous lambda functions to apply transformations element-wise across an entire DataFrame.    
...
```{python}
# remove newly created columns 
pd_courses.drop(columns = ["platform_dwell_length", "platform_start_year","studied_programming"], inplace = True)
```
...
```{r, echo = FALSE, results = 'markup'}
#-----benchmark dplyr and data.table functions-----  
mb <- microbenchmark(mutate(student_courses, platform_dwell_length = difftime(platform_end_date, platform_start_date, units = "days")),   
                     dplyr_case_when = mutate(student_courses, studied_programming = case_when(str_detect(online_course, "^R_") ~ "Studied R",
                                                                                               str_detect(online_course, "^Python_") ~ "Studied Python",
                                                                                               TRUE ~ "No")),    
                     student_courses[, "platform_dwell_length" := .(difftime(platform_end_date, platform_start_date, units = "days"))],
...
output_diff_days = 10 * timeit.timeit(apply_diff_days, number = 100)
output_programming = 10 * timeit.timeit(apply_programming, setup = setup_programming, number = 100)
```

```{r, echo = FALSE, results = 'markup'}
pandas_mb <- data.frame("expr" = c('pd_courses["platform_dwell_length"]',
                                   'pd_courses["studied_programming"]'),
                        "mean" = c(py$output_diff_days, 
                                   py$output_programming),
                        "neval" = rep(100, 2))    
