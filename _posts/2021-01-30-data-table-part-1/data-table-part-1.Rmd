...
**Note:** Selecting columns is a computationally expensive operation for R and Python Pandas data frames as data are stored as rows.     
...
```{r, echo = FALSE, results = 'markup'}
#-----benchmark dplyr and data.table functions-----  
mb <- microbenchmark(mutate(student_courses, platform_dwell_length = platform_end_date - platform_start_date),
...
```{r, echo = FALSE, results = 'markup'}
#-----benchmark dplyr and data.table functions-----  
mb <- microbenchmark(student_courses %>% group_by(student_id) %>% summarise(total_courses = n()),                                               student_courses[, .(total_courses = .N), by = student_id], 
...
output_groupby_nunique = 10 * timeit.timeit(groupby_nunique, setup = setup_apply_lan, number = 100)    
output_groupby_two_cols = 10 * timeit.timeit(groupby_two_cols, setup = setup_apply_lan, number = 100)    
```
