...
A simple introduction to group by operations has already been covered [here](https://erikaduan.github.io/posts/2021-01-30-data-table-part-1/#simple-group-by-operations). This post further explores how different outputs can be obtained by performing `data.table` group by operations in different ways.   
...
Imagine that you are interested in how the length of time spent on a platform varies per student per platform. The solution this similar to the one above, except that transformations are specified for selective variable(s) and more than one variable is specified within a group.    


## Using `dplyr`    

In `dplyr`, this selection is facilitated by using `across()` inside `summarise()`, which allows you to perform the same list of functions on a single or multiple columns.    
...
In `data.table`, the equivalent method is to list variable(s) of interest inside `.SDcols` and perform aggregations using `lapply(.SD, ...)`.    
...
As expected, the `data.table` operations are faster than `dplyr` operations, although there is a non-linear performance decrease when multiple functions are evaluated for a subset of columns.   
...
Another insight you might be interested in is the number of times a student switches between a platform. Deriving this is a multiple step process:  
...
**Note:** I am currently experiencing a package dependency error caused by `dtplyr` which prevents `.list()` functions from evaluating inside `data.table` objects, so the code below has been disabled in this document and exists for demonstration purposes only.   
